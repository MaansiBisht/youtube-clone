...
export const getSearchPageVideos = createAsyncThunk(
    "youtube/App/getSearchPageVideos",
    async (isNext, { getState }) => {
      try {
        ...
        const searchTerm = state.youtubeSlice.searchTerm;
        const response = await axios.get(
          `${BASEURL}search?&q=${searchTerm}&key=${API_KEY}&part=snippet&type=video&${isNext ? `pageToken=${nextPageTokenFromState}` : ""}`
        );
